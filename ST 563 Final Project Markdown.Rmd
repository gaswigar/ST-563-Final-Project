...
namecheck <- c("fixed.acidity","volatile.acidity","citric.acid","residual.sugar","chlorides","free.sulfur.dioxide","total.sulfur.dioxide","density","pH","sulphates","alcohol","quality","quality2cat","quality3cat")

red_train2 <- red_train
names(red_train2) <- namecheck
red_test2 <- red_test
names(red_test2) <- namecheck

bagfit <- randomForest(quality ~.-quality2cat-quality3cat, data=red_train2,mtry=11,importance=TRUE)

yhat.bag = predict(bagfit, newdata=red_test2)
mean((yhat.bag-red_test2$quality)^2)
importance(bagfit)
varImpPlot(bagfit)
```
...
set.seed(1)
for(i in 1:10){
  rffit <- randomForest(quality ~.-quality2cat-quality3cat,data=red_train2,mtry=i,importance=TRUE)
  yhat.rf = predict(rffit,newdata=red_test2)
  rfmse <- mean((yhat.rf-red_test2$quality)^2)
  if(rfmse < bestmse){
    bestmse = rfmse
    bestfitcount = i
    bestrf = rffit
...
svm1 <- train(quality2cat ~. -quality, 
              data = red_train,  
              method = "svmLinear", 
...
test_pred_svm1 <- predict(svm1,
                    newdata = red_test,
                    type = "raw")

conf_svm1<-confusionMatrix(
  data = test_pred_svm1, 
  reference = red_test$quality2cat)

svm1_results<-data.frame(Type="Linear",
                         Accuracy=conf_svm1$overall[1],
                         PreCision=conf_svm1$byClass[5],
                         Recall=conf_svm1$byClass[6]) 
```
...
svm2 <- train(quality2cat ~. -quality, 
              data = red_train,  
              method = "svmRadial", 
              trControl = train_control, 
              preProcess = c("center","scale"), 
...
test_pred_svm2 <- predict(svm2,
                    newdata = red_test,
                    type = "raw")

conf_svm2<-confusionMatrix(
  data = test_pred_svm2, 
  reference = red_test$quality2cat)

svm2_results<-data.frame(Type="Radial",
                         Accuracy=conf_svm2$overall[1],
                         PreCision=conf_svm2$byClass[5],
                         Recall=conf_svm2$byClass[6]) 
```
...
```{r Polynomial SVM}
set.seed(21)
svm3 <- train(quality2cat ~. -quality, 
              data = red_train, 
              method = "svmPoly", 
              trControl = train_control, 
              preProcess = c("center","scale"), 
...
test_pred_svm3 <- predict(svm3,
                    newdata = red_test,
                    type = "raw")

conf_svm3<-confusionMatrix(
  data = test_pred_svm3, 
  reference = red_test$quality2cat)

svm3_results<-data.frame(Type="Polynomial",
                         Accuracy=conf_svm3$overall[1],
                         PreCision=conf_svm3$byClass[5],
                         Recall=conf_svm3$byClass[6]) 
```

```{r SVM Results}
svm_results<-bind_rows(svm1_results,
                       svm2_results,
                       svm3_results) %>%
  mutate(MissClass=1-Accuracy) %>%
  kable()
